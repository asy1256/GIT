...
HRESULT bulletB::init(float x, float y, OBJECT obj, int roomnum, int spawn, int* plX, int* plY, bool* blankshot)
{
	character::init();
	...
	_ch.roomnum = roomnum;
	_ch.obj = obj;
	_ch.img = IMAGEMANAGER->findImage("bandana");
	_ch.gun = IMAGEMANAGER->findImage("bPistol");
...
void bulletB::move(void)
{
	...
	else
	{
		if (200 < getDistance(_ch.x, _ch.y, _Tile[*_bk.plY][*_bk.plX].rc.left + TILESIZE / 2, _Tile[*_bk.plY][*_bk.plX].rc.top + TILESIZE / 2))
		{
			...
			_ch.x += cosf(_bk.moveangle) * 3;
			_ch.y += -sinf(_bk.moveangle) * 3;
			_bk.moving = true;
